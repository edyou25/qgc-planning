...
message_counts = {'heartbeat': 0, 'position': 0, 'mission': 0, 'other': 0}  # 消息计数器
enable_diagnostic = True  # 是否启用诊断功能
...
if enable_diagnostic:
    ...
    diagnostic_t = threading.Thread(target=diagnostic_thread_func, daemon=True)
    diagnostic_t.start()
    print("诊断线程已启动")
    add_status_message("诊断线程已启动，将定期检查连接状态")
